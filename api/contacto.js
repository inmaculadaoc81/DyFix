...
export default async function handler(req,res){
  ...
  try{
    ...
    const subject=`Nueva consulta DysonTech - dysontech.es | ${nombre}`;
    const html=`<div style="font-family:Arial,sans-serif;line-height:1.6;color:#161B28"><h2 style="color:#2E7AD1">Nueva consulta desde DysonTech</h2><p><strong>Nombre:</strong> ${safe.nombre}</p><p><strong>Teléfono:</strong> ${safe.telefono}</p><p><strong>Email:</strong> ${safe.email}</p><p><strong>Producto Dyson:</strong> ${safe.modelo}</p><p><strong>Descripción de la avería:</strong><br>${safe.mensaje}</p><hr><p style="font-size:12px;color:#5B6675">Enviado desde el formulario de dysontech.es</p></div>`;
    const mime=[`From: DysonTech <${from}>`,`To: ${to}`,`Reply-To: ${email}`,`Subject: ${encodeHeader(subject)}`,'MIME-Version: 1.0','Content-Type: text/html; charset="UTF-8"','Content-Transfer-Encoding: 8bit','',html].join('\r\n');
